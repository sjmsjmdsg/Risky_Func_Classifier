...
class PythonHandler(LanguageHandler):
    def __init__(self):
        ...
        PYTHON_LANGUAGE = Language(ts_py.language())
        parser = Parser()
        parser.language = PYTHON_LANGUAGE  # Set the language
    ...
    def extract_functions(self, source_code):
        ...
        def walk(node, class_name=None, depth=0):
            ...
            if node.type == 'class_definition':
                ...
                # Recursively traverse child nodes (functions might be nested inside)
                for child in node.children:
    ...
    def get_full_name(self, node):
        ...
        if node.type == 'identifier':
            ...
            value = self.get_full_name(value_node) if value_node else ''
